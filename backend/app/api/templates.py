...
def get_templates(db: Session = Depends(get_db)) -> list[Template]:
    return list_templates(db)


@router.post("", response_model=TemplateOut, status_code=201)
def post_template(body: TemplateCreate, db: Session = Depends(get_db)) -> Template:
    try:
...
def post_clone(
    template_id: int, body: TemplateClone | None = None, db: Session = Depends(get_db)
) -> Template:
    try:
...
def put_template(template_id: int, body: TemplateCreate, db: Session = Depends(get_db)) -> Template:
    try:
...
def post_preview(template_id: int, body: PreviewRequest, db: Session = Depends(get_db)) -> dict:
