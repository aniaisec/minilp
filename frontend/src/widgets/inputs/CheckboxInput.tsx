...
// Multi-select checkbox (§2.1). Value shape: string[]. allow_other supported.
export function CheckboxInput(props: InputWidgetProps) {
  const { input, hotkeys, value, onChange } = props;
  const opts = resolveOptions(input, hotkeys);
  const selected = Array.isArray(value) ? value : [];
  return (
    <div className="mlp-field" role="group" aria-label={input.label} data-testid={`input-${input.id}`}>
      ...
      <div className="mlp-options-list">
        {opts.map((opt) => (
          <button
            ...
            role="checkbox"
            aria-checked={selected.includes(opt.raw)}
