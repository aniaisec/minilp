...
- **Annotator home (M5 landing → M8 home):** the annotator's default screen, listing every project they can work on with the volume waiting in each — labels available, open units, their own contribution so far, and *why* a project is unavailable when it is (paused, below `min_reputation`, or simply drained). Two presentations of one list, toggled and remembered: the existing **table** (dense, scannable, sorts by remaining work) and a **card grid** (one card per project with its work counts and a fill bar, comfortable on a tablet and on a phone). Reached at a **stable route** — `/` with an annotator but no project — so it is somewhere you can *return to*, not merely where you start.
- **Exit to home (M8):** every project screen carries a visible way back — the annotation view, the review queue, and the per-project admin view. Leaving **releases the held lease** rather than stranding it until expiry (§2.7), so the unit is immediately available to someone else. An unsubmitted answer prompts before discarding; a submitted one never blocks the exit.
...
*Deliverables:*
- **Merge & routing (§7.2):** calibration-weighted merge (weights = live reputation), declarative pipeline stages, auto-finalize thresholds, human review queue UI (reviewer-role gated), `final_labels` provenance, `review.queue_backlog` / `project.completed` webhook events.
- **Annotator home (§11):** the M5 landing page promoted to a real home screen — a stable route an annotator can return to, listing every project with the work waiting in it. Adds a **card-grid view** beside the existing table (per-project: labels available, open units, your labels, a fill bar, and the blocked reason when there is one), a view toggle persisted like the theme and auto-submit preferences, and an empty state that distinguishes "nothing exists yet" from "you have finished everything".
- **Exit to home (§11):** a `Home`/`Exit` control on every project screen — annotation view, review queue, per-project admin. Exiting the annotation view **releases the lease** through the existing `skip` path (variant retained, §2.7) instead of leaving a slot leased until it expires; an unsubmitted answer warns first. `Esc` is the natural key, but it is already the reserved "clear selection" key (§2.4), so the exit hotkey is chosen without colliding — or the control stays mouse/tab-reachable only.

*Acceptance:* synthetic judges with known accuracies → merge weights converge; a disagreeing unit routes to review and an override is recorded with provenance; the backlog webhook fires past threshold. The home page's per-project counts **reconcile exactly with `/tasks/available`**, which already applies the same exclusion the assignment engine does — table and cards render from one fetch and can never disagree. Exiting a project mid-task reopens its slot with the variant intact (asserted in the DB, not just the UI) and the annotator lands back on home with the counts updated. Keyboard-only round trip: home → project → label → exit → home, zero mouse events.
